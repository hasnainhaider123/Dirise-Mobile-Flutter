...
class _SingleStoreScreenState extends State<SingleStoreScreen> {
  ...
  filterProduct({productId}) {
    repositories.postApi(url: ApiUrls.filterByPriceUrl, context: context,
      mapData: {
  ...
  Future getCategoryStores({required int page, String? search, bool? resetAll}) async {
    ...
    await repositories.getApi(url: "${ApiUrls.vendorProductListUrl}$url&key=fedexRate&zip_code=${locationController.zipcode.value.toString()}&pagination=5&page=$paginationPage").then((value) {
      paginationLoading.value = false;
    ...
    await repositories.getApi(url: "${ApiUrls.vendorProductListUrl}$url&device_id=${deviceId.toString()}&key=fedexRate&zip_code=${locationController.zipcode.value.toString()}&pagination=25&page=$paginationPage").then((value) {
      paginationLoading.value = false;
      isDataLoading.value = true;
      modelProductsList.vendorProducts!.data ??= [];
      final response = ModelStoreProducts.fromJson(jsonDecode(value));
          ...
          slivers: [
            ...
            if (modelProductsList.vendorProducts!.data != null && controller.isFilter.value == false )
              isDataLoading.value
                ...
                //   );
                // }
                    if(modelProductsList.vendorProducts!.data!.length != 1 && modelProductsList.vendorProducts!.data!.length == 9) {
                      return SliverGrid.builder(
                        gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(
                          crossAxisCount: 1,
                          crossAxisSpacing: 10,
